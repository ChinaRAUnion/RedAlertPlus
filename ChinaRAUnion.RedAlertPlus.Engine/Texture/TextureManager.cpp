...
namespace
{
	...
	void ExtractDDSInfo(const byte* src, size_t size, const DDS_HEADER*& ddsHeader, std::unique_ptr<byte[]>& bitData, size_t& bitDataSize)
	{
		ThrowIfNot(*(const uint32_t*)(src) == DDS_MAGIC, L"Invalid dds file.");
		auto hdr = reinterpret_cast<const DDS_HEADER*>(src + sizeof(uint32_t));
		ThrowIfNot(hdr->size == sizeof(DDS_HEADER) && hdr->ddspf.size == sizeof(DDS_PIXELFORMAT), L"Invalid dds file.");
		...
		ThrowIfNot(size >= offset, L"Invalid dds file.");
		ddsHeader = hdr;
		auto pBitData = src + offset;
		bitDataSize = size - offset;
		bitData = std::make_unique<byte[]>(bitDataSize);
		memcpy_s(bitData.get(), bitDataSize, pBitData, bitDataSize);
	}
...
Texture TextureManager::CreateTexture2D(const byte * data, size_t size, const std::wstring & contentType)
{
	if (contentType == L"image/dds")
	{
		...
		ExtractDDSInfo(data, size, header, bitData, bitDataSize);
		...
		if ((header->ddspf.flags & DDS_FOURCC) && (MAKEFOURCC('D', 'X', '1', '0') == header->ddspf.fourCC))
		{
			...
			if (arraySize == 0)
				ThrowIfFailed(HRESULT_FROM_WIN32(ERROR_INVALID_DATA));
		...
		}
		{
			...
			std::vector<D3D12_SUBRESOURCE_DATA> initData(subresourceCount);

			size_t skipMip = 0;
			size_t twidth = 0;
			size_t theight = 0;
			size_t tdepth = 0;
			ThrowIfFailed(FillInitData(width, height, depth, mipCount, arraySize, format, INT_MAX, bitDataSize, bitData.get(),
				twidth, theight, tdepth, skipMip, initData.data()));

			auto handle = AllocateHandle();

			Microsoft::WRL::ComPtr<ID3D12Resource> texture;
			ThrowIfFailed(CreateD3DResources(_deviceContext.D3DDevice, resDim, twidth, theight, tdepth, mipCount - skipMip, arraySize,
				format,
				false , initData.data(), &texture, handle.first));
			...
			_deviceContext.D3DDevice->CreateShaderResourceView(texture.Get(), &srvDesc, handle.first);

			DataToUpload toUpload;
			toUpload.data = std::move(bitData);
			toUpload.resData = std::move(initData);


			const UINT64 uploadBufferSize = GetRequiredIntermediateSize(texture.Get(), 0, subresourceCount);

			CD3DX12_HEAP_PROPERTIES uploadHeapProperties(D3D12_HEAP_TYPE_UPLOAD);
			ThrowIfFailed(_deviceContext.D3DDevice->CreateCommittedResource(
			...
			Texture result(texture.Get(), handle.first, handle.second, twidth, theight);
	...
	else
	{
		...
		// 将纹理上载到 GPU。
		{
			...
			toUpload.resData.emplace_back();
			auto& textureData = toUpload.resData.back();
...
void TextureManager::Upload(Texture & texture, ID3D12GraphicsCommandList * commandList, std::vector<WRL::ComPtr<IUnknown>>& resourcesWaitForUpload)
{
	...
	UpdateSubresources(commandList, texture.Resource, texture.dataForUpload->resForUpload.Get(), 0, 0, texture.dataForUpload->resData.size()
		, texture.dataForUpload->resData.data());
